...
Checkpoint:
bun --cwd apps/poller test
must validate the fixture without calling the live API.
...
Important: reserve one PostgreSQL connection and use that same connection to:
...
Create two scheduled jobs with command:
bun run poll
...
- Seven DolarAPI casas are validated.
