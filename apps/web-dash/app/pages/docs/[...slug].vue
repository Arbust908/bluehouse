<template>
      ...
      <article class="min-w-0">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-3">
          ...
          <span v-if="doc.status" class="group/status relative inline-flex">
            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold focus-visible:outline-indigo-600" :class="statusClass" tabindex="0" aria-describedby="status-tooltip">
        ...
        <p class="mt-4 max-w-prose text-sm leading-relaxed text-zinc-600 dark:text-slate-300">{{ doc.summary }}</p>
  ...
  <main v-else class="mx-auto max-w-3xl px-5 py-20 text-center">
    ...
    <NuxtLink to="/docs" class="mt-6 inline-flex min-h-10 items-center rounded-md bg-indigo-600 px-4 text-sm font-semibold text-zinc-50 hover:bg-indigo-700 focus-visible:outline-indigo-600">Back to documentation</NuxtLink>
  </main>
</template>
