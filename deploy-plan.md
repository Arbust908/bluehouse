...
│      │
│ There is no web/API service yet. The poller needs a Docker image so Dokploy can deploy it and run scheduled jobs (bun run poll) via docker exec. Dokploy requires a running container for scheduled jobs — the container itself doesn't auto-poll on startup; it idles with sleep infinity.                          │
│      │
│ What we're building                                             │
│      │
...
│ 4. docker run --rm bluehouse-poller:local bun run db:migrate connects to local Postgres and runs migrations                │
│ 5. docker run --rm bluehouse-poller:local bun run poll fetches rates, inserts into local DB                                │
